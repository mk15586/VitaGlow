...
import { Search, LocateFixed, MapPin, Navigation, Star } from 'lucide-react';
...
// MapSection is now a client-only component for the map
...
function NearbyFacilitiesPageImpl() {
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined') {
            Promise.all([
                ...
                import('lucide-react').then(mod => mod.Pill),
                import('lucide-react').then(mod => mod.Stethoscope)
            ]).then(([L, Hospital, Pill, Stethoscope]) => {
...
export default dynamic(() => Promise.resolve(NearbyFacilitiesPageImpl), { ssr: false });